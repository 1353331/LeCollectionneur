...
namespace LeCollectionneur.Modeles
{
    class ConversationADO
    {
        ...
        //Variable pour savoir si l'utilisateur est Utilisateur1 en Bd
        public static void EnvoyerMessageStatic2(Message message, Conversation convo)
        {
            //"INSERT INTO `messages` (`Date`, `Message`, `IdConversation`, `IdUtilisateur`) VALUES ( '" + message.Date.ToString("yyyy-MM-dd hh:mm:ss")+"', '"+message.Contenu+"', '"+Convo.Id+"', '"+message.idUtilisateur+"');";
            string req = "INSERT INTO `messages` (`Date`,`Message`,`Conversation_Id`,`item`,`image`,`emoji`,`utilisateur_Id`) " +
                "VALUES('" + DateTime.UtcNow.ToString("yyyy-MM-dd hh:mm:ss") + "' , '" + message.Contenu + "'," + convo.Id + "," + message.item + "," + false + "," + false + "," + UtilisateurADO.utilisateur.Id + ");";
            BD.Commande(req);
        }
        ...
        public static bool HasConversation(int User)
        {

            string req = "SELECT * FROM `conversations` WHERE(Utilisateur1_Id = " + UtilisateurADO.utilisateur.Id + " AND Utilisateur2_Id = " + User + ") OR(Utilisateur2_Id = " + UtilisateurADO.utilisateur.Id + " AND Utilisateur1_Id = " + User + ");";
            var temp = BD.Selection(req);

            return temp.Tables[0].Rows.Count != 0;
        }
